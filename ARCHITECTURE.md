...
Define el lenguaje común que hablan todos los módulos. No tiene dependencias de otros módulos hermanos.
...
*   **`src/main/java/com/example/lector/capture/`**
    *   **`CameraXImageCapturer.kt`** (Clase): Implementa `ImageCapturer`.
        ...
        *   **Mejoras Fase 2**: Usa `ResolutionSelector` con estrategia de fallback para asegurar compatibilidad con dispositivos diversos.
        *   **Funciones clave**:
            *   `startCapture()`: Inicia `Preview` y `ImageAnalysis`.
...
Punto de entrada de la aplicación. Conoce a todos los módulos y los conecta.

*   **`src/main/java/com/example/lector/`**
    *   **`MainActivity.kt`** (Activity):
        *   **Responsabilidad**: Inyección de dependencias manual, manejo de permisos y conexión del pipeline reactivo.
        *   **Flujo**: Conecta el `Flow` de captura -> procesador -> decodificador -> UI.
        *   **Manejo de Errores**: Implementa `retry(3)` para la captura y `catch` global para mostrar errores al usuario.
        *   **Instrumentación**: Logs `Perf` midiendo el tiempo de procesamiento y decodificación.
...
4.  **Presentación (Main Thread)**:
    *   `MainActivity` recibe el `QrResult`.
    *   Se muestra un `Toast` y un log en consola con el contenido del QR.

## 5. Entorno de Construcción (Build System)

Para garantizar la reproducibilidad en cualquier entorno Windows sin dependencias previas, el proyecto incluye un sistema de construcción portable:

*   **`tools/`**: Directorio que contiene las distribuciones binarias de las herramientas.
    *   `jdk/`: OpenJDK 17.
    *   `gradle/`: Gradle 8.5.
*   **`setup_env.ps1`**: Script de PowerShell que configura temporalmente las variables de entorno (`JAVA_HOME`, `PATH`) para usar las herramientas portables de `tools/`.

**Flujo de Compilación Portable:**
```powershell
. .\setup_env.ps1      # Carga las variables
gradle :app:installDebug # Compila e instala
```
